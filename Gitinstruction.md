...
```
### Удаление веток
Если ветка болasdasdasьше не нужна, ее можно удалить, для этого используйте команду `git branch -d <название ветки>`. Ветку не возможно удалить находять в ней, для этого нужно выйти в сонвную ветку *master* командой `git branch master`. Master - ветка по умолчанию.
